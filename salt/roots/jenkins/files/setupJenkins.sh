...
#create ssh key.
docker exec {{ jenkins_container }} bash -c "[ -f /var/jenkins_home/.ssh/id_rsa ] || ssh-keygen -q -N '' -t rsa  -f /var/jenkins_home/.ssh/id_rsa"
...
if [ x"$findit" = x"false" ]; then
    docker exec ${JENKINS_NAME} bash -c  "cat /var/jenkins_home/.ssh/id_rsa.pub | ssh -i '${SSH_KEY_PATH}' -p 29418 ${GERRIT_ADMIN_UID}@${GERRIT_SSH_HOST} gerrit create-account --group 'Non-Interactive Users' --full-name 'Jenkins Server' --ssh-key - jenkins "
